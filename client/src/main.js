...
const renderRegister = () => {
  ...
  document.getElementById('registerForm').onsubmit = async e => {
    e.preventDefault()
    const [name, email, password] = Array.from(e.target.elements).map(
      el => el.value
    )
    showLoading()
    ...
    app.innerHTML = `
      <div class="min-h-screen flex flex-col bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
        <header class="bg-white dark:bg-gray-800 shadow p-4 flex justify-between items-center">
          <h1 class="text-2xl font-bold">Task Dashboard — ${me.name}, ${me.email}</h1>
...
const renderTaskDetail = task => {
    ...
    container.innerHTML = `
        <div class="space-y-4 max-w-xl">
            ...
            <p class="text-sm text-gray-500">Created by: ${task.user?.name || 'Unknown'}</p>
            <p class="text-sm text-gray-500">Created at: ${formatDate(task.created_at)}</p>
            <p class="text-sm text-gray-500">Updated at: ${formatDate(task.updated_at)}</p>
